...
class HostConfig
{
    ...
    public function __construct(array $data = [])
    {
        ...
        if (array_key_exists('AutoRemove', $data)) {
            $this->autoRemove = (bool) $data['AutoRemove'];
        }
        if (isset($data['Tmpfs']) && is_array($data['Tmpfs'])) {
            $this->tmpfs = [];
            foreach ($data['Tmpfs'] as $path => $options) {
                if (is_string($path) && is_string($options)) {
     ...
     */
    public function setTmpfs(array $tmpfs): self
    {
        $this->tmpfs = $tmpfs;
    ...
    public function toArray(): array
    {
        ...
        if ($this->autoRemove !== null) {
            $payload['AutoRemove'] = $this->autoRemove;
        }

        if ($this->tmpfs !== null) {
            $payload['Tmpfs'] = $this->tmpfs;
        }
